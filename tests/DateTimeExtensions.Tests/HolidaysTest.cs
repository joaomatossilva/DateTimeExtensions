...
namespace DateTimeExtensions.Tests
{
    ...
    public class HolidaysTest
    {
        ...
        {
            var firstDayInMonth = new NamedDay("test1", new NthDayOfWeekInMonthDayResolver(1, DayOfWeek.Sunday, 1,
                CountDirection.FromFirst));
            Assert.AreEqual(new DateTime(2012, 1, 1), firstDayInMonth.GetInstance(2012));

            var eigththDayInMonth = new NamedDay("test2", new NthDayOfWeekInMonthDayResolver(2, DayOfWeek.Sunday, 1,
                CountDirection.FromFirst));
            Assert.AreEqual(new DateTime(2012, 1, 8), eigththDayInMonth.GetInstance(2012));


            var lastDayInMonth = new NamedDay("test3", new NthDayOfWeekInMonthDayResolver(1, DayOfWeek.Tuesday, 1,
                CountDirection.FromLast));
            Assert.AreEqual(new DateTime(2012, 1, 31), lastDayInMonth.GetInstance(2012));


            var twentyfourthDayInMonth = new NamedDay("test4", new NthDayOfWeekInMonthDayResolver(2, DayOfWeek.Tuesday, 1,
                CountDirection.FromLast));
        ...
        public void easter()
        {
            var easterHoliday = new NamedDay("easter1", new EasterBasedDayResolver(0));
            Assert.AreEqual(new DateTime(2012, 4, 8), easterHoliday.GetInstance(2012));

            var esterPositiveOffsetHoliday = new NamedDay("easter2", new EasterBasedDayResolver(5));
            Assert.AreEqual(new DateTime(2012, 4, 13), esterPositiveOffsetHoliday.GetInstance(2012));

            var esterNegativeOffsetHoliday = new NamedDay("easter3", new EasterBasedDayResolver(-5));
        ...
        {
            var fixedHoliday = new NamedDay("fixed1", new FixedDayResolver(new DayInYear(2, 15)));
            Assert.AreEqual(new DateTime(2012, 2, 15), fixedHoliday.GetInstance(2012));

            var fixedHoliday2 = new NamedDay("fixed1", new FixedDayResolver(2, 16));
