		...
		public void default_working_days() {
			...
			for (int i = 0; i < 7; i++) {
				...
				if (testDate.DayOfWeek == DayOfWeek.Saturday || testDate.DayOfWeek == DayOfWeek.Sunday) {
					Assert.IsFalse(testDate.IsWorkingDay(foo_ci), Enum.GetName(typeof(DayOfWeek), testDate.DayOfWeek) + " is not be a working day by default");
				} else {
					Assert.IsTrue(testDate.IsWorkingDay(foo_ci), Enum.GetName(typeof(DayOfWeek), testDate.DayOfWeek) + " is be a working day by default");
				}
			}
